...
prim_int(PRIM_PROTOTYPE)
{
    ...
    if ((!(oper1->type == PROG_FLOAT)) ||
        (oper1->type == PROG_FLOAT
         && arith_good((double) oper1->data.fnumber))) {
        result =
            (int) ((oper1->type ==
                    PROG_OBJECT) ? oper1->data.objref : (oper1->type ==
                                                         PROG_FLOAT) ? oper1->
                   data.fnumber : oper1->data.number);
    } else {
        result = 0;
        fr->error.error_flags.i_bounds = 1;
    }
...
prim_dist2d(PRIM_PROTOTYPE)
{
    ...
    if (!no_good(oper1->data.fnumber)) {
        if (oper1->data.fnumber < 0.0) {
            ...
            fresult = sqrt((double) pow((double)
                                        (oper3->type ==
                                         PROG_INTEGER ? oper3->data.
                                         number : oper3->data.number),
                                        2) + pow((double) (oper4->type ==
                                                           PROG_INTEGER ?
                                                           oper4->data.
                                                           number : oper4->data.
                                                           number), 2));
        ...
        if (oper1->data.fnumber < 0.0) {
            ...
            fresult = sqrt((double) pow((double)
                                        ((oper3->type ==
                                          ...
                                          number : oper3->data.number) -
                                         (oper1->type ==
                                          PROG_INTEGER ? oper1->data.
                                          number : oper1->data.number)),
                                        2) +
                           pow((double)
                               ((oper4->type ==
                                 PROG_INTEGER ? oper4->data.number : oper4->
                                 data.number) - (oper2->type ==
                                                 PROG_INTEGER ? oper2->data.
                                                 number : oper2->data.number)),
                               2));
        }
    } else {
        fresult = 0.0;
...
prim_3ddist(PRIM_PROTOTYPE)
{
    ...
    if (!no_good(oper1->data.fnumber)) {
        if (oper1->data.fnumber < 0.0) {
            ...
            fresult = sqrt((double) pow((double)
                                        ((oper4->type ==
                                          PROG_INTEGER ? oper4->data.
                                          number : oper4->data.number) -
                                         (oper1->type ==
                                          PROG_INTEGER ? oper1->data.
                                          number : oper1->data.number)),
                                        2) +
                           pow((double)
                               ((oper5->type ==
                                 PROG_INTEGER ? oper5->data.number : oper5->
                                 data.number) - (oper2->type ==
                                                 PROG_INTEGER ? oper2->data.
                                                 number : oper2->data.number)),
                               2) +
                           pow((double)
                               ((oper6->type ==
                                 PROG_INTEGER ? oper6->data.number : oper6->
                                 data.number) - (oper3->type ==
                                                 PROG_INTEGER ? oper3->data.
                                                 number : oper3->data.number)),
                               2));
        }
