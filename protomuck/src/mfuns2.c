    ...
    if (!string_compare(argv[0], "me")) {
         ...
         if(ref == NOTHING) {
            for(result = pcount(); result && ref == NOTHING; result--)
               if (string_prefix(PNAME(pdbref(result)), argv[0])) ref = pdbref(result);
         }
    }
    sprintf(buf, "#%d", ref);
    return buf;
...
mfn_lmatch(MFUNARGS)
{
   /* {lmatch:list,match,delim} */
    ...
    while (*ptr && !(string_prefix(ptr, argv[1]) &&
        ...
        if(string_prefix(ptr, argv[1]) && *ptr && i2 == 0) i2 = i;
...
mfn_force(MFUNARGS)
{
    ...
    do {
        ...
        if (nxt) *nxt++ = '\0';
        force_level++;
        if (*ptr) process_command(dbref_first_descr(obj), obj, ptr);
        force_level--;
        ptr = nxt;
    } while (ptr);
    *buf = '\0';
