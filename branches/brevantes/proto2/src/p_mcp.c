...
muf_mcp_callback(McpFrame *mfr, McpMesg *mesg, McpVer version, void *context)
{
    ...
    for (ptr = DBFETCH(obj)->sp.program.mcpbinds; ptr; ptr = ptr->next) {
    ...
    if (ptr) {
        ...
        tmpfr = interp(descr, user, getloc(user), obj, -1, PREEMPT,
                       STD_REGUID, 0);
        if (tmpfr) {
            ...
            for (arg = mesg->args; arg; arg = arg->next) {
                if (!arg->value) {
                    ...
                    argval.data.array =
                        new_array_packed(mcp_mesg_arg_linecount
                                         (mesg, arg->name));
    ...
    while (result) {
        ...
        switch (argval->type) {
                ...
                while (contd) {
                    ...
                    switch (subval->type) {
                        ...
                        case PROG_OBJECT:
                            ...
                            break;
                        case PROG_FLOAT:
                            sprintf(buf, "%.15g", subval->data.fnumber);
                            mcp_mesg_arg_append(msg, argname.data.string->data,
                                                buf);
                            break;
                        default:
            ...
            case PROG_OBJECT:
                ...
                break;
            case PROG_FLOAT:
                sprintf(buf, "%.15g", argval->data.fnumber);
                mcp_mesg_arg_remove(msg, argname.data.string->data);
                mcp_mesg_arg_append(msg, argname.data.string->data, buf);
                break;
            default:
...
prim_mcp_bind(PRIM_PROTOTYPE)
{
    ...
    if (!ptr) {
        ...
        ptr->next = DBFETCH(program)->sp.program.mcpbinds;
        DBFETCH(program)->sp.program.mcpbinds = ptr;
    }
...
fbgui_muf_event_cb(GUI_EVENT_CB_ARGS)
{
    ...
    while (name) {
        ...
        for (i = 0; i < lines; i++) {
            ...
            temp4.data.string =
                alloc_prog_string(gui_value_get(dlogid, name, i));
    ...
    if (lines > 0) {
        ...
        for (i = 0; i < lines; i++) {
            ...
            temp4.type = PROG_STRING;
            temp4.data.string =
                alloc_prog_string(mcp_mesg_arg_getline(msg, "data", i));
        ...
        for (i = 0; i < count; i++) {
            ...
            switch (temp2->type) {
                ...
                case PROG_OBJECT:
                    ...
                    break;
                case PROG_FLOAT:
                    sprintf(buf, "%.15g", temp2->data.fnumber);
                    value = buf;
                    break;
                default:
                    while (i-- > 0) {
