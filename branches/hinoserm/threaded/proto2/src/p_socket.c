...
muf_socket_sendevent(struct muf_socket_queue *curr)
{
    ...
    if (!curr->theSock->readWaiting && curr->pid == curr->fr->pid) {
        ...
        if (curr->theSock->sslStatus != 1 ) {
            ...
            free((void *) curr->theSock->sslError);
        }
#endif
...
handle_ssl_error(struct muf_socket *mufSock, int *IOreturn)
{
    ...
    if (ssl_error != SSL_ERROR_WANT_READ
        ...
        /* Copy the value of IOreturn, because we're changing it later. */
        mufSock->sslStatus = *IOreturn; /* Triggers SOCKET.SSLERR event */
        switch (ssl_error) {
...
#ifdef WIN_VC
        ioctl(oper[1].data.sock->socknum, FIONBIO, 0);
#else
        fcntl(oper[1].data.sock->socknum, F_SETFL, 0);
#endif
...
#if defined(SSL_SOCKETS) && defined(USE_SSL)
        if (oper[1].data.sock->ssl_session) {
            ssl_write_loop(oper[1].data.sock, &result, buf);
        } else
#endif
    ...
    if (oper[0].data.sock->rawmode) {
        ...
        *mystring = '\0';
        ...
        if ( (FD_ISSET(oper[0].data.sock->socknum, &reads))
            ...
            if (gotmessage && oper[0].data.sock->usequeue) { /* special handling */
                char *p, *pend, *q, *qend;
                struct muf_socket *theSock = oper[0].data.sock;
...
prim_sockclose(PRIM_PROTOTYPE)
{
    ...
    if (oper[0].data.sock->is_player) { /* don't close descrs */
        result = 0;
    } else {
#if defined(SSL_SOCKETS) && defined(USE_SSL)
        ...
        if (oper[0].data.sock->ssl_session) {
            if (SSL_shutdown(oper[0].data.sock->ssl_session) < 1)
                SSL_shutdown(oper[0].data.sock->ssl_session);
        }
#endif
        if (shutdown(oper[0].data.sock->socknum, 2) == -1)
...
prim_sockshutdown(PRIM_PROTOTYPE)
{
    ...
    if (oper[1].data.sock->is_player && tmp == 2) { /* don't close descrs */
        result = 0;
    } else {
        if (shutdown(oper[1].data.sock->socknum, tmp) == -1)
#if defined(BRAINDEAD_OS)
            result = -1;
#else
            result = errnosocket;
        ...
        if (tmp == 2) {
            ...
            /* Alynna - Must be done twice to enact a bidirectional shutdown,
               so do it again if still open, meaning remote end allows socket reuse */
            if (oper[1].data.sock->ssl_session) {
                if (SSL_shutdown(oper[1].data.sock->ssl_session) < 1)
                    SSL_shutdown(oper[1].data.sock->ssl_session);
            }
#endif
            oper[1].data.sock->connected = 0; /* only say not-connected if */
            remove_socket_from_queue(oper[1].data.sock); /* complete shutdown.  */
        }
    }
...
prim_nbsock6open(PRIM_PROTOTYPE)
{
    ...
    if (!(myhost = gethostbyname2(oper[1].data.string->data, AF_INET6))) {
        ...
        name.sin6_port = (int) htons(oper[0].data.number);
...
prim_sockcheck(PRIM_PROTOTYPE)
{
    ...
    oper[0].data.sock->last_time = time(NULL);
    result = oper[0].data.sock->connected;
    PushInt(result);
}
...
prim_lsockopen(PRIM_PROTOTYPE)
{
    ...
    errors = listen(sockdescr, oper[1].data.number);
    if (errors == -1) {
    ...
    result->data.sock->commands = 0;
    result->data.sock->is_player = 0;
    result->data.sock->port = oper[0].data.number; /* listening port */
...
if (oper[0].data.sock->ipv6) {
    ...
    result->type = PROG_SOCKET;
    result->data.sock = (struct muf_socket *) malloc(sizeof(struct muf_socket));
    hu = host_getinfo6(remoteaddr6.sin6_addr, oper[0].data.sock->port,
    ...
    result->data.sock->connected_at = time(NULL);
    result->data.sock->last_time = time(NULL);
    result->data.sock->port = oper[0].data.sock->port; /* port it connected to */
    result->data.sock->usequeue = 0;
    result->data.sock->usesmartqueue = 0;
    result->data.sock->commands = 0;
    result->data.sock->rawmode = 0;
...
prim_socket_setuser(PRIM_PROTOTYPE)
{
    ...
    if (oper[2].data.sock->listening)
        abort_interp("Does not work with listening sockets. (1)");
    if (!oper[2].data.sock->connected)
        abort_interp("This socket is not connected.");
    if (oper[2].data.sock->is_player)
        abort_interp("This socket already connected to a player or descr.");
    theSock = oper[2].data.sock;
    if (oper[1].type != PROG_OBJECT || !valid_player(&oper[1]))
...
prim_set_sockopt(PRIM_PROTOTYPE)
{
    ...
    if (oper[1].type != PROG_SOCKET)
        abort_interp("Expected MUF socket. (1)");
    theSock = oper[1].data.sock;
    if (oper[0].type != PROG_INTEGER)
        abort_interp("Expected integer. (2)");
    flag = oper[0].data.number;
...
prim_udpsend(PRIM_PROTOTYPE)
{
 ...
 sendto(tmp, oper[2].data.string->data, oper[2].data.string->length, 0, (struct sockaddr *)&sa, sizeof(sa)); 
