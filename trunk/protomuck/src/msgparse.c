...
{
    if (Arch(what) && TArch(OWNER(what))) return 1;
    return 0;
}
...
Wizperms(dbref what)
{
    if (Wiz(what) && TWiz(OWNER(what))) return 1;
    return 0;
}
...
/******** HOOK ********/
char   *
mesg_parse(int descr, dbref player, dbref what, dbref perms, const char *inbuf, char *outbuf, int maxchars, int mesgtyp)
{
