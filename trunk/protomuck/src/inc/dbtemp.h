...
/*
 * $Header: /export/home/davin/tmp/protocvs/protomuck/src/inc/dbtemp.h,v 1.1.1.1 2000-09-18 01:04:35 akari Exp $
...
#define RawMLevel(x)	( ( ((FLAGS(x) & W3)?1:0)<<2 ) + \
			  ( ((FLAGS(x) & W2)?1:0)<<1 ) + \
			  ( ((FLAGS(x) & W1)?1:0)    )	)

#define MLevel(x)	( Man(x) ? LMAN : RawMLevel(x) )

#define QLevel(x)	( (FLAGS(x) & QUELL) ? 0 : MLevel(x) )

#define SetMLevel(x,y)	( FLAGS(x) =  ( FLAGS(x) & ~(W1|W2|W3) ) | \
					  ( ((y) & 4) ? W3 : 0 ) | \
					  ( ((y) & 2) ? W2 : 0 ) | \
					  ( ((y) & 1) ? W1 : 0 )   )
