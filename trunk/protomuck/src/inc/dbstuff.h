...
/*
 * $Header: /export/home/davin/tmp/protocvs/protomuck/src/inc/dbstuff.h,v 1.1.1.1 2000-09-18 01:04:35 akari Exp $
 * $Log: not supported by cvs2svn $
 * Revision 1.3  1996/09/19 07:13:58  jtraub
 * removed KILL_OK flag (renamed to UNDEF_FLAG_1)
 *
 * Revision 1.2  1996/09/19 05:46:56  jtraub
 * removed inserver hacks for HackAndSlash code
 *
 */
...
#define W3		  	   0x10 /* wizard level three */
...
#define F2ETHEREAL	    0x8 /* Ethereal object, new dark concept */
...
#define F2MCP              0x80 /* Program is a MUCK C Program. */
...
#define MAN	(1)
#define Man(x)	((x) == MAN || (FLAGS(x) & W4) ) /* Either char #1, or W4 flagged chars get Man power */

#define RawMLevel(x)	( ( ((FLAGS(x) & W4)?1:0)<<4 ) + \
 			        ( ((FLAGS(x) & W3)?1:0)<<2 ) + \
 			        ( ((FLAGS(x) & W2)?1:0)<<1 ) + \
			        ( ((FLAGS(x) & W1)?1:0)    )   )

#define MLevel(x)	( Man(x) && !(FLAGS(x) & W4) ? LMAN : RawMLevel(x) )
...
#define Light(x) ((FLAGS(x) & LINK_OK) && !(Typeof == TYPE_PROGRAM))

#define Builder(x) ( !tp_restricted_building || (FLAGS(x) & BUILDER) || TMage(x))

#define Guest(x) ( (FLAG2(x) & F2GUEST) && !TMage(x) )

#define Vacation(x) ((FLAGS(x) & VEHICLE) && (Typeof == TYPE_PLAYER))
...
#define Viewable(x) ((FLAGS(x) & VEHICLE) && !(Typeof == TYPE_THING) && !(Typeof == TYPE_PLAYER))
...
#define PROG_ARRAY       13     /* NeonMuck array type */
#define PROG_SOCKET      14     /* NeonMuck socket type */

#define MAX_VAR         56	/* maximum number of variables including the
				 * basic ME and LOC                */
...
struct muf_array_elem {
   struct inst *val;
   struct muf_array_elem *next;
};

struct muf_array {
   int size;
   int links;
   struct muf_array_elem *elems;
};
...
struct inst {			/* instruction */
	...
	struct muf_array *array;/* a neonmuck array */
