...
reg_user_is_barred(int site, int user)
{
    ...
    if (TMage(user) || get_property_class(user, "@/id"))
        return FALSE;
    return (!opensite);
}
...
reg_alternation_match(char *trix, const char *str)
{
    ...
    while (*trix) {
        ...
        /* dislike fns that gratuitously modify input:  */
        {
            ...
            if (reg_wildcard_match(wildcard, str, TRUE))
                return TRUE;
...
siteMatch(int site, int template1)
{
    ...
    if (!(template1 & 0x000000FF))
        mask = 0x000000FF;
    if (!(template1 & 0x0000FFFF))
        mask = 0x0000FFFF;
    if (!(template1 & 0x00FFFFFF))
        mask = 0x00FFFFFF;
    if (!(template1 & 0xFFFFFFFF))
        mask = 0xFFFFFFFF;
