...
gui_initialize(void)
{
    ...
    mcp_package_register(GUI_PACKAGE, minver, maxver, gui_pkg_callback, NULL,
                         NULL);
}
...
gui_pkg_callback(McpFrame *mfr, McpMesg *msg, McpVer ver, void *context)
{
    ...
    if (!string_compare(msg->mesgname, "ctrl-value")) {
        ...
        int i;
        const char **value =
            (const char **) malloc(sizeof(const char *) * valcount);
        ...
        if (dat->callback) {
            dat->callback(dat->descr, dlogid, id, evt, msg, did_dismiss,
                          dat->context);
        }
...
gui_dlog_closeall_descr(int descr)
{
    ...
    while (ptr) {
        ...
        if (ptr->callback) {
            ...
            ptr->callback(ptr->descr, ptr->id, "_closed", "buttonpress", &msg,
                          1, ptr->context);
            mcp_mesg_clear(&msg);
        }
        ptr = ptr->next;
...
int
GuiListInsert(const char *dlogid, const char *id, int after, int lines,
              const char **value)
{
    McpMesg msg;
    McpFrame *mfr;
...
int
GuiMenuItem(const char *dlogid, const char *id, const char *type,
...
int
GuiMenuCheckBtn(const char *dlogid, const char *id, const char *name,
                const char **args)
{
...
gui_ctrl_make_l(const char *dlogid, const char *type, const char *pane,
                const char *id, const char *text, const char *value, int layout,
                ...)
{
    va_list ap;
    McpMesg msg;
...
int
GuiEdit(const char *dlogid, const char *pane, const char *id, const char *text,
        const char *value, int width, int layout)
{
    char buf[32];

    sprintf(buf, "%d", width);
    return gui_ctrl_make_l(dlogid, "edit", pane, id, text, value, layout,
                           "width", buf, NULL);
}


int
GuiText(const char *dlogid, const char *pane, const char *id, const char *value,
        int width, int layout)
{
    char widthbuf[32];

    sprintf(widthbuf, "%d", width);
    return gui_ctrl_make_l(dlogid, "text", pane, id, NULL, value, layout,
                           "width", widthbuf, NULL);
}


int
GuiSpinner(const char *dlogid, const char *pane, const char *id,
           const char *text, int value, int width, int min, int max, int layout)
{
    char widthbuf[32];
    char valbuf[32];
    char minbuf[32];
    ...
    return gui_ctrl_make_l(dlogid, "spinner", pane, id, text, valbuf, layout,
                           "width", widthbuf, "min", minbuf, "max", maxbuf,
                           NULL);
}


int
GuiCombo(const char *dlogid, const char *pane, const char *id, const char *text,
         const char *value, int width, int editable, int layout)
{
    ...
    return gui_ctrl_make_l(dlogid, "combobox", pane, id, text, value, layout,
                           "width", buf, "editable", editable ? "1" : "0",
                           NULL);
}


int
GuiMulti(const char *dlogid, const char *pane, const char *id,
         const char *value, int width, int height, int fixed, int layout)
{
    char widthbuf[32];
    char heightbuf[32];
    ...
    return gui_ctrl_make_l(dlogid, "multiedit", pane, id, NULL, value, layout,
                           "width", widthbuf,
                           "height", heightbuf, "font",
                           (fixed ? "fixed" : "variable"), NULL);
}


int
GuiHRule(const char *dlogid, const char *pane, const char *id, int height,
         int layout)
{
    char heightbuf[32];

    sprintf(heightbuf, "%d", height);
    return gui_ctrl_make_l(dlogid, "hrule", pane, id, NULL, NULL, layout,
                           "height", heightbuf, NULL);
}


int
GuiVRule(const char *dlogid, const char *pane, const char *id, int thickness,
         int layout)
{
    char widthbuf[32];

    sprintf(widthbuf, "%d", thickness);
    return gui_ctrl_make_l(dlogid, "vrule", pane, id, NULL, NULL, layout,
                           "width", widthbuf, NULL);
}
...
int
GuiGroupBox(const char *dlogid, const char *pane, const char *id,
            const char *text, int collapsible, int collapsed, int layout)
{
...
int
GuiButton(const char *dlogid, const char *pane, const char *id,
          const char *text, int width, int dismiss, int layout)
{
    char widthbuf[32];

    sprintf(widthbuf, "%d", width);
    return gui_ctrl_make_l(dlogid, "button", pane, id, text, NULL, layout,
                           "width", widthbuf, "dismiss", dismiss ? "1" : "0",
                           NULL);
}
...
{
    struct dlogidlist *item =
        (struct dlogidlist *) malloc(sizeof(struct dlogidlist));
...
{
    const char *keywords[] =
        { "Misc.", "Wedding", "Party", "Toading", "New MUCK" };
    const char *dlg =
        GuiSimple(descr, "A demonstration dialog", post_dlog_cb, NULL, NULL);
    ...
    GuiMulti(dlg, NULL, "body", NULL, 80, 12, 1,
             GUI_NSEW | GUI_VEXP | COLSPAN(2));
