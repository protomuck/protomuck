...
check_contents(dbref obj)
{
    ...
    if ((whattype == TYPE_PROGRAM) ||
        (whattype == TYPE_EXIT) || (whattype == TYPE_GARBAGE))
        return;
...
void
add_event(int descr, dbref player, dbref loc, dbref trig, int dtime,
          dbref program, struct frame *fr, const char *strdata)
{
}
